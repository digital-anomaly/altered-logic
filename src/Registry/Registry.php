...
final class Registry
{
     ...
     * @param EmbedGatedBatchIdentityDTO $identity   The identity of the gated batch to get.
     * @return EmbedGatedBatch
     */
    public static function getEmbedGatedBatch(
        bool $isDeferred,
        EmbedGatedBatchIdentityDTO $identity,
    ): EmbedGatedBatch {

        $key = $identity->buildKey();

        $buildNew = fn(): EmbedGatedBatch => new EmbedGatedBatch($identity);
     ...
     *
     * @param boolean                            $isDeferred Whether the batch is deferred or not.
     * @param DocSearchableGatedBatchIdentityDTO $identity   The identity of the gated batch to get.
     * @return DocSearchableGatedBatch
     */
    public static function getDocSearchableGatedBatch(
        bool $isDeferred,
        DocSearchableGatedBatchIdentityDTO $identity,
    ): DocSearchableGatedBatch {

        $key = $identity->buildKey();

        $buildNew = fn(): DocSearchableGatedBatch => new DocSearchableGatedBatch($identity);
